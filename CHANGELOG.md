...
### Fixed
- Texto de período no Termo de Alerta e no Relatório de Gestão Fiscal agora respeita o **Tipo de Período** selecionado ("Só Quadrimestral", "Só Semestral" ou "Ambos"). Antes, o trecho "e Xº Semestre" era sempre exibido mesmo quando o usuário selecionava "Só Quadrimestral".

### Changed
- `label_periodo_completo` (`src/rgf_limites.py`) recebe o novo parâmetro `tipo_periodo` e retorna apenas o quadrimestre, apenas o semestre, ou ambos, conforme o valor informado.
- `gerar_alerta_docx` e `gerar_relatorio_docx` (`src/rgf_report_builder.py`) recebem o parâmetro `tipo_periodo` e propagam a escolha da página até o texto final do documento.
